...
def get_grays(image, width, height):
    ...
    >>> import os
    >>> test_filename = os.path.join(os.path.dirname(__file__), 'perceptualHash-test.jpg')
    ...
    if wand is None and PIL is None:
        raise ImportError('must have wand or Pillow/PIL installed to use perceptualHash on images')
...
def perceptualHash_row_col(image, size=8):
    ...
    >>> row, col = perceptualHash_row_col([0,0,1,1,1, 0,1,1,3,4, 0,1,6,6,7, 7,7,7,7,9, 8,7,7,8,9], size=4)
    >>> format(row, '016b')
    '0100101111010001'
    >>> format(col, '016b')
    '0101001111111001'
    ...
    >>> test_filename = os.path.join(os.path.dirname(__file__), 'perceptualHash-test.jpg')
    >>> with wand.image.Image(filename=test_filename) as image:
    ...     row, col = perceptualHash_row_col(image)
...
def perceptualHash_int(image, size=8):
    """Calculate row and column difference hash for given image and return
    hashes combined as a single 2*size*size bit integer (row_hash in most
    significant bits, col_hash in least).

    >>> perceptualHash_int([0,0,1,1,1, 0,1,1,3,4, 0,1,6,6,7, 7,7,7,7,9, 8,7,7,8,9], size=4)
    1272009721
    """
    row_hash, col_hash = perceptualHash_row_col(image, size=size)
...
def format_bytes(row_hash, col_hash, size=8):
    """Format perceptualHash integers as binary string of size*size//8 bytes (row_hash
...
def format_hex(row_hash, col_hash, size=8):
    """Format perceptualHash integers as hex string of size*size//2 total hex digits
...
def format_matrix(hash_int, bits='01', size=8):
    """Format perceptualHash integer as matrix of bits.
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--size', type=int, default=8,
                        help='width and height of perceptualHash image size, default %(default)d')
    ...
    def load_image(filename):
        if wand is not None:
            ...
            sys.stderr.write('You must have wand or Pillow/PIL installed to use the perceptualHash command\n')
            sys.exit(1)
    ...
    if len(args.filename) == 0:
        ...
        if args.format == 'grays':
            ...
            row_hash, col_hash = perceptualHash_row_col(image, size=args.size)
            if args.format == 'hex':
        ...
        hash1 = perceptualHash_int(image1, size=args.size)
        hash2 = perceptualHash_int(image2, size=args.size)
